const Img = ({ url, caption }: any) => {
  return (
    <div>
      <img src={url} />
      {caption.length ? (
        <p className="w-full text-center my-3 md:mb-12 text-base text-gray-400">
          {caption}
        </p>
      ) : (
        ""
      )}
    </div>
  );
};

const List = ({ items, style }: any) => {
  return (
    <ol
      className={`pl-5 ${style === "ordered" ? "list-decimal" : "list-disc"}`}
    >
      {items.map((listItem: any, index: any) => {
        return (
          <li key={index} dangerouslySetInnerHTML={{ __html: listItem }}></li>
        );
      })}
    </ol>
  );
};

const Quote = ({ quote, caption }: any) => {
  return (
    <div className="bg-purple/10 p-3 pl-5 border-l-4 border-purple-400">
      <p className="text-xl leading-10 md:text-2xl">{quote}</p>
      {caption.length ? (
        <p className="w-full text-purple-400 text-base">{caption}</p>
      ) : (
        ""
      )}
    </div>
  );
};

const EventContent = ({ block }: any) => {
  let { type, data } = block;

  if (type == "paragraph") {
    return <p dangerouslySetInnerHTML={{ __html: data.text }}></p>;
  }

  if (type == "header") {
    if (data.level == 3) {
      return (
        <h3
          className="text-3xl font-bold"
          dangerouslySetInnerHTML={{ __html: data.text }}
        ></h3>
      );
    }
    return (
      <h2
        className="text-4xl font-bold"
        dangerouslySetInnerHTML={{ __html: data.text }}
      ></h2>
    );
  }

  if (type == "image") {
    return <Img url={data.file.url} caption={data.caption} />;
  }

  if (type == "quote") {
    return <Quote quote={data.text} caption={data.caption} />;
  }

  if (type == "list") {
    return <List style={data.style} items={data.items} />;
  }
};

export default EventContent;
